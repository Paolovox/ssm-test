...
export class ClassiComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('classiTable') classiTable: OGListComponent;
  data: any;
  router$: Subscription;
  search$: Subscription;
  ...
  selectOptions = {
    aree_list: Array<{ id: string, text: string }>()
  };
  ...
  ngOnInit() {
    this.pageTitleService.setTitle('Classi', '');
  ...
  getData(reset = false, loading = true) {
    ...
    this.classiTable.clearSelection();
    const obj: Rest = {
      path: `${this.path}`,
      type: 'GET'
    };
  ...
  delete(id: string, name: string) {
    this.dialog.openConfirm('Elimina classe', 'Sei sicuro di voler eliminare la classe '
  ...
  dataModal(data: any): Observable<any> {
    if (data.aree_list) {
      this.selectOptions.aree_list = data.aree_list;
    }
    return new Observable((observer) => {
      this.ogModal.openModal('Classe', data.nome ? '' : 'Aggiungi una classe', data)
        .subscribe((res: any) => {
