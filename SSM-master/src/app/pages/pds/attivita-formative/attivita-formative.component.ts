...
@Component({
  selector: 'app-attivita-formative',
  templateUrl: './attivita-formative.component.html',
  styleUrls: ['./attivita-formative.component.scss']
})
export class AttivitaFormativeComponent implements OnInit, OnDestroy {

  path = 'pds_attivita_formative';

  @ViewChild('attivitaFormativeTable') attivitaFormativeTable: OGListComponent;
  data: any;
  router$: Subscription;
  search$: Subscription;
  ...
  constructor(
    ...
    // TODO: Eliminare se inutile
  }

  ngOnInit() {
    this.pageTitleService.setTitle('Tipologia', '');
  ...
  getData(reset = false, loading = true) {
    ...
    this.attivitaFormativeTable.clearSelection();
    const obj: Rest = {
      path: `${this.path}`,
      type: 'GET'
    };
    ...
    this.main.rest(obj)
      .then((res: any) => {
        ...
        if (reset) {
          this.attivitaFormativeTable.firstPage();
        }
      }, () => {
      });
  }
  ...
  delete(id: string, name: string) {
    this.dialog.openConfirm('Elimina tipologia', 'Sei sicuro di voler eliminare la tipologia '
  ...
  dataModal(data: any): Observable<any> {
    if (data.classi_list) {
      this.selectOptions.classi_list = data.classi_list;
    }
    return new Observable((observer) => {
      this.ogModal.openModal('Tipologia', data.nome ? '' : 'Aggiungi una tipologia', data)
        .subscribe((res: any) => {
