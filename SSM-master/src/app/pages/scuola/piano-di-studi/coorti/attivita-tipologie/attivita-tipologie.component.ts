...
@Component({
  selector: 'app-attivita-tipologie',
  templateUrl: './attivita-tipologie.component.html',
  styleUrls: ['./attivita-tipologie.component.scss']
})
export class AttivitaTipologieComponent implements OnInit, OnDestroy {

  path = 'registrazioni_tipologie_attivita';
  idScuola: string;
  idCoorte: string;

  @ViewChild('attivitaTipologieTable') attivitaTipologieTable: OGListComponent;
  ...
  settings: OGListSettings = {
    columns: [
      ...
      {
        column: 'nome_settore_scientifico',
        name: 'Nome settore scientifico',
  ...
  selectOptions = {
    settoriScientificiList: Array<{ id: string, text: string }>()
  };
  dialogFields: Array<DialogFields> = [
    {
      type: 'INPUT',
      placeholder: 'Nome tipologia attività',
      name: 'nome'
    },
    {
      type: 'SELECT',
  ...
  ngOnInit() {
    ...
    this.pageTitleService.setTitle('Tipologie attività', '');
  ...
  getData(reset = false, loading = true) {
    ...
    this.attivitaTipologieTable.clearSelection();
    const obj: Rest = {
    ...
    this.main.rest(obj)
      .then((res: any) => {
        ...
        if (reset) {
          this.attivitaTipologieTable.firstPage();
        }
      }, () => {
        if (loading) {
          this.main.loaderOff();
  ...
  delete(id: string, name: string) {
    this.dialog.openConfirm('Elimina tipologia attività', 'Sei sicuro di voler eliminare la tipologia ' + name + '?', 'ELIMINA', 'Annulla')
      .then(() => {
        const obj: Rest = {
          type: 'DELETE',
          path: `${this.path}/${this.idScuola}/${this.idCoorte}/${id}`
  ...
  dataModal(data: any): Observable<any> {
    ...
    return new Observable((observer) => {
      this.ogModal.openModal('Scheda tipologia attività', '', data)
        .subscribe((res: any) => {
