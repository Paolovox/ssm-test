...
@Component({
  selector: 'app-attivita-filtri',
  templateUrl: './attivita-filtri.component.html',
  styleUrls: ['./attivita-filtri.component.scss']
})
export class AttivitaFiltriComponent implements OnInit, OnDestroy {

  path = 'registrazioni_filtri';
  idScuola: string;
  idCoorte: string;
  idAttivita: string;

  @ViewChild('attivitaFiltroTable') attivitaFiltroTable: OGListComponent;
  ...
  getData(reset = false, loading = true) {
    ...
    this.attivitaFiltroTable.clearSelection();
    const obj: Rest = {
    ...
    this.main.rest(obj)
      .then((res: any) => {
        ...
        if (reset) {
          this.attivitaFiltroTable.firstPage();
        }
      }, () => {
        if (loading) {
          this.main.loaderOff();
