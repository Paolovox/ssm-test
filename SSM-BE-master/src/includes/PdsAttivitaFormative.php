...
$app->group('/pds_attivita_formative', function (RouteCollectorProxy $groupAree) use ($auth) {

  $data = [
    "table" => "ssm.ssm_pds_attivita_formative",
    "id" => "id",
    "sort" => "nome",
    "order" => "asc",
  ...
  $groupAree->get('', function (Request $request, Response $response, $args) use ($auth, $crud) {
      $p['_ssm.ssm_pds_attivita_formative.idstatus'] = 1;
  ...
  $groupAree->put('', function (Request $request, Response $response, $args) use ($crud) {
    ...
    $retValidate = validate( "pds_attivita_formative", $p );
    if( $retValidate != "" ) {
